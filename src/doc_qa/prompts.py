"""Questions, system instruction, and per-turn prompt builders."""
...
- Use ONLY the document content provided in this conversation.
...
- Be long, detailed and hierarchical especially when the document actually contains
  enough detail.
...
def _answer_contract(question_index: int, question_text: str) -> str:
    return f"""
Requirements for this question:
- Start with: "## Q{question_index}: {question_text}"
- Then write the answer to the question, unless "{NOT_FOUND}"
- Then include the most relevant subsections from the allowed labels list.
- If a subsection is not supported by the document, write: {NOT_FOUND}
...
def build_first_prompt(
    ...
    """First turn: carries the document itself. Sent once per document."""
    return f"""
Document ID: {doc_id}

You will answer questions using ONLY the document below.

Question:
{question_text}

{_answer_contract(question_index, question_text)}
...
def build_followup_prompt(question_index: int, question_text: str) -> str:
    """Later turns: the document is already in the session, so only the question is sent."""
    return f"""
Answer the next question using ONLY the document provided earlier in this conversation.
Do not use outside knowledge, common sense, or assumptions.
